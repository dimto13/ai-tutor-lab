...
const personas = collectPersonaIds(JSON.parse(await readFile(personaPath, "utf8")) as PersonaDocument);
const glossary = JSON.parse(await readFile(glossaryPath, "utf8")) as GlossaryDocument;
const glossaryKeys = new Set((glossary.concepts ?? []).flatMap((concept) => (concept.key ? [concept.key] : [])));
...
if (issues.length > 0) {
  ...
  console.log(`✓ persona audit: ${scenarioFiles.length} scenarios have audience and glossary coverage`);
}
