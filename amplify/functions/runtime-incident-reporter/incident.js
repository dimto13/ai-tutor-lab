...
function cleanText(value) {
  ...
  let result = value;
  for (const pattern of [...secretPatterns, ...piiPatterns]) result = result.replace(pattern, REDACTED);
...
export function createIncidentGate({ maxEvents = 20, windowMs = 60_000, failureThreshold = 5 } = {}) {
  ...
  return Object.freeze({
    admit(now = Date.now()) {
      ...
      if (events.length >= maxEvents) return Object.freeze({ allowed: false, reason: 'rate-limited' });
      events.push(now);
