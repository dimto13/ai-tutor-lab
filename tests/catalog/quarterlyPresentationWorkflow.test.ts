...
test(
  "quarterly presentation workflow keeps A/B evidence on one shared synthetic basis",
  async () => {
...
test(
  "quarterly presentation workflow exposes the required comparison dimensions",
  async () => {
    const scenario = await readJson<Scenario>(scenarioPath);
...
test(
  "management presentation skill is reusable and keeps invention guards explicit",
  async () => {
    const scenario = await readJson<Scenario>(scenarioPath);
