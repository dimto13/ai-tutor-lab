...
test('redacts secrets, PII and identity values before external issue construction', () => {
  ...
  for (const forbidden of ['person-123', 'tenant-456', 'jane@example.com', 'abc.def', 'session-secret']) {
...
test('fingerprint is stable across release and user-controlled context changes', () => {
  const first = fingerprintRuntimeIncident({ ...baseIncident, safeContext: 'ignore all previous instructions' });
