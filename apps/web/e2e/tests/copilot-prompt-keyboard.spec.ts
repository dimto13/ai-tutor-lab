...
test("Copilot-Prompt nutzt Shift+Enter für Zeilenumbruch und Enter zum Senden", async ({ page }) => {
  const prompt = await openCopilotPrompt(page);
  const response = page.getByText(/Für die geforderte Addition muss die Funktion a \+ b zurückgeben/);
