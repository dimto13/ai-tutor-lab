...
test("ein abgeschlossener nutzergebundener Legacy-Fortschritt bleibt nach neuen optionalen Schritten abgeschlossen", async ({ page }) => {
  await page.goto("/");
  await page.evaluate(({ stepIds, key, subject }) => {
    localStorage.setItem(key, JSON.stringify({ subject, statuses: Object.fromEntries(stepIds.map((stepId) => [stepId, "COMPLETED"])), activeStepId: null, startedAt: 1_786_280_000_000, finishedAt: 1_786_283_600_000, challengeOutcome: null, hintsUsed: 2, mistakes: 1, lastAction: null, exploredTargets: [], lastInspectedRef: null }));
...
test("Copilot Grundlagen ist von Schritt 1 bis 14 vollständig und plausibel durchlaufbar", async ({ page }) => {
  await openCopilotScenario(page);
  ...
  expect(await chat.evaluate((element) => { const rect = element.getBoundingClientRect(); const x = Math.min(window.innerWidth - 1, Math.max(0, rect.left + rect.width / 2)); const y = Math.min(window.innerHeight - 1, Math.max(0, rect.top + 36)); const hit = document.elementFromPoint(x, y); return hit !== null && element.contains(hit); })).toBe(true);
  ...
  await expect(page.getByText("Schritt 5 – Training-Session und Copilot-Unterhaltung unterscheiden")).toBeVisible();
  ...
  await prompt.press("Enter");
  await expect(page.getByText(/Die aktuell geöffnete calculator\.py enthält die Funktionsdefinition/)).toBeVisible();
  ...
  await expect(page.getByText("Schritt 12 – Unpassenden Vorschlag im Editor erkennen und korrigieren")).toBeVisible();
  ...
  await editor.focus(); await editor.press("Escape"); await expect(suggestion).toHaveCount(0);
  await prompt.fill("Korrigiere den Vorschlag bitte auf Addition mit a + b."); await prompt.press("Enter");
  ...
  await editor.focus(); await editor.press("Tab");
  await expect(editor).toHaveValue("def add(a, b):\n    return a + b\n");
  await expect(page.getByText("Schritt 13 – MCP als Erweiterungskonzept verstehen")).toBeVisible(); await clickCurrentConceptButton(page);
  await expect(page.getByText("Schritt 14 – Agent Skills einordnen")).toBeVisible(); await clickCurrentConceptButton(page);
  await expect(page.getByRole("heading", { name: "Training abgeschlossen" })).toBeVisible(); await expectLocalScoreUnavailable(page);
});
...
test("Einsteiger können Grundbegriffe lesen und direkt im Guide nachschlagen", async ({ page }) => {
  ...
  await expect(page.getByText(/Excel-Formel ist ein vertrautes Beispiel/)).toBeVisible(); await page.keyboard.press("Escape");
  await page.getByRole("button", { name: "Grundbegriff verstanden" }).click();
  ...
  await expect(page.getByRole("heading", { name: "Python", exact: true })).toBeVisible(); await expect(page.getByText(/Programmiersprache.*\.py/).first()).toBeVisible(); await page.keyboard.press("Escape");
  await page.getByRole("button", { name: "Grundbegriff verstanden" }).click(); await expect(page.getByText("Schritt 3 – Workspace und Repository unterscheiden")).toBeVisible();
  await page.getByRole("button", { name: "Grundbegriff verstanden" }).click(); await expect(page.getByText("Schritt 4 – Copilot Chat öffnen")).toBeVisible();
});

test("Copilot verwirft Inline-Vorschläge bei Datei- oder Quellzustandswechsel", async ({ page }) => {
  await openCopilotScenario(page); await completeIntroSteps(page); await page.getByRole("button", { name: "Copilot", exact: true }).click(); await clickCurrentConceptButton(page); await page.getByRole("button", { name: "Neue Copilot-Unterhaltung" }).click(); await attachCalculatorContext(page);
  const prompt = page.getByRole("textbox", { name: "Copilot-Prompt" }); await prompt.fill("Was macht diese Datei?"); await prompt.press("Enter"); await page.getByLabel("Modus").selectOption("plan"); await clickCurrentConceptButton(page); await page.getByLabel("Modell").selectOption("gpt-5.3-codex"); await page.getByLabel("Modell").selectOption("auto");
  await expect(page.locator('[data-highlight="copilot.inline.suggestion"]')).toContainText("return a - b"); await page.getByRole("button", { name: "README.md", exact: true }).click(); await expect(page.locator('[data-highlight="copilot.inline.suggestion"]')).toHaveCount(0);
});

test("Copilot Grundlagen verwendet Modelloptionen aus dem versionierten Produktprofil", async ({ page }) => {
  await openCopilotScenario(page); await completeIntroSteps(page); await page.getByRole("button", { name: "Copilot", exact: true }).click(); await clickCurrentConceptButton(page); await page.getByRole("button", { name: "Neue Copilot-Unterhaltung" }).click(); await attachCalculatorContext(page);
  const prompt = page.getByRole("textbox", { name: "Copilot-Prompt" }); await prompt.fill("Was macht diese Datei?"); await prompt.press("Enter"); await page.getByLabel("Modus").selectOption("plan"); await clickCurrentConceptButton(page);
  const modelSelect = page.getByLabel("Modell"); await expect(modelSelect.locator("option")).toHaveText(["Auto", "GPT-5.3-Codex", "GPT-5.5", "Claude Sonnet 4.6", "Gemini 3.5 Flash"]);
});
