...
const toolTrainings: TrainingCardModel[] = [
  ...
  {
    id: "github-basics", scenarioId: null, title: "GitHub – Grundlagen",
    description: "Repositories, Branches, Remotes, Pull Requests und die Zusammenarbeit auf GitHub als eigenes Werkzeug kennenlernen.", icon: Github, available: true, label: "Source Control Platform · 3 Modi",
    modes: [
  ...
  {
    id: "github-copilot-basics", scenarioId: null, title: "GitHub Copilot – Grundlagen",
    description: "Chat, Inline-Vorschläge, Kontext, Arbeitsmodi, Modellauswahl und kontrollierte Übernahme von KI-Vorschlägen kennenlernen.", icon: Bot, available: true, label: "AI Coding Assistant · 3 Modi",
    modes: [
  ...
  },
  {
    ...
    description: "Claude Code in einer vollständig simulierten Terminalumgebung sicher einsetzen: Auftrag und Plan kontrollieren, Berechtigungen bewusst entscheiden, Fehler korrigieren und den Endzustand mit Tests selbst verifizieren.", icon: Terminal, available: true, label: "CLI Agent · 3 Modi",
    modes: [
...
const workflowTrainings: TrainingCardModel[] = [
  {
    id: "developer-workflow-basics", scenarioId: null, title: "VS Code, Git & GitHub Copilot – Zusammenspiel",
  ...
  {
    id: "research-workflow", scenarioId: null, title: "Mit KI recherchieren und Quellen prüfen",
    description: "Eine Recherche als eigenen KI-Workflow üben: Auftrag formulieren, Ergebnis erzeugen, Quellen prüfen und das Resultat fachlich bewerten.", icon: Search, available: true, label: "AI Workflow · 3 Modi",
    modes: [
  ...
  {
    id: "table-data-workflow", scenarioId: null, title: "Tabellendaten mit KI auswerten und plausibilisieren",
    description: "Tabellendaten als eigenen KI-Workflow analysieren: Daten und Annahmen prüfen, Auswertung nachvollziehen, Auffälligkeiten erkennen und das Ergebnis fachlich verifizieren.", icon: Search, available: true, label: "AI Workflow · 3 Modi",
    modes: [
      { scenarioId: "table-data-workflow.explore", label: "Explore", description: "Daten, Auswertung und Kontrollpunkte frei erkunden", icon: Search, multiplier: "×0,5" },
      { scenarioId: "table-data-workflow.guided", label: "Guided", description: "Tabellenauswertung Schritt für Schritt plausibilisieren", icon: RouteIcon, multiplier: "×1,0" },
      { scenarioId: "table-data-workflow.challenge", label: "Challenge", description: "Fehlende Region selbstständig erkennen und verifizieren", icon: Target, multiplier: "×2,0" },
    ],
  },
  {
    id: "html-page-workflow", scenarioId: null, title: "HTML-Seite mit KI erstellen und iterativ verbessern",
    description: "Eine Seite als eigenen Workflow erzeugen, sichtbar weiterentwickeln, zwischen Vorschau und Quelltext wechseln und unbeabsichtigte Änderungen erkennen.", icon: Code2, available: true, label: "AI Workflow · 3 Modi",
    modes: [
...
const aiWorkflowTrainings = aiWorkflowModuleLine.moduleIds.map((moduleId) => {
  ...
  if (!training) throw new Error(`Missing dashboard training card for module line module: ${moduleId}`);
  return training;
});

const otherTrainings: TrainingCardModel[] = [
  {
    id: "m365-copilot-basics", scenarioId: null, title: "M365 Copilot Grundlagen",
    description: "Den Microsoft-365-Copilot-Chat mit freigegebenem Work-Kontext, klaren Arbeitsaufträgen, Quellenprüfung und expliziter menschlicher Freigabe einsetzen. Alle Inhalte sind synthetisch.", icon: Bot, available: true, label: "Office Assistant · 3 Modi",
    modes: [
...
function Dashboard() {
  return (
    <div className="min-h-screen bg-background">
      <header className="border-b border-border bg-panel"><div className="mx-auto flex h-14 max-w-6xl items-center gap-3 px-6"><GraduationCap className="h-5 w-5 text-accent" /><span className="text-sm font-semibold tracking-tight text-foreground">AI Training Lab</span><div className="ml-auto"><AccountMenu /></div></div></header>
...
function TrainingSection({ title, description, trainings }: { title: string; description: string; trainings: TrainingCardModel[] }) {
  return <section className="mt-10"><div><h2 className="text-base font-semibold text-foreground">{title}</h2><p className="mt-1 text-[13px] leading-relaxed text-muted-foreground">{description}</p></div><div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3">{trainings.map((training) => <TrainingCard key={training.id} training={training} />)}</div></section>;
}

function TrainingCard({ training }: { training: TrainingCardModel }) {
  const Icon = training.icon;
  return <article className="min-w-0 flex flex-col rounded-xl border border-border bg-card p-5 transition-colors hover:border-ring/60"><div className="flex min-w-0 items-center gap-2"><span className="flex h-8 w-8 items-center justify-center rounded-md bg-accent/15"><Icon className="h-4 w-4 text-accent" /></span><span className="min-w-0 break-words text-[11px] uppercase tracking-wider text-muted-foreground">{training.label ?? (training.steps ? `${training.steps} Schritte` : "Training")}</span>{training.steps ? <span className="ml-auto text-[11px] text-muted-foreground">{training.steps} Schritte</span> : null}</div><h3 className="mt-3 text-base font-semibold leading-snug text-foreground">{training.title}</h3><p className="mt-2 flex-1 text-[13px] leading-relaxed text-muted-foreground">{training.description}</p>{training.modes ? <div className="mt-4 space-y-2">{training.modes.map((mode) => <ModeAction key={mode.scenarioId} mode={mode} />)}</div> : <SingleTrainingAction training={training} />}</article>;
}

function ModeAction({ mode }: { mode: TrainingModeOption }) {
  const percent = useStoredProgressPercent(mode.scenarioId) ?? 0;
  const Icon = mode.icon;
  return <Link to="/training/$scenarioId" params={{ scenarioId: mode.scenarioId }} className="flex min-w-0 items-center gap-3 rounded-lg border border-border px-3 py-2.5 transition-colors hover:border-ring hover:bg-white/5"><Icon className="h-4 w-4 shrink-0 text-accent" /><span className="min-w-0 flex-1"><span className="flex items-center gap-2 text-sm font-medium text-foreground">{mode.label} <span className="text-[10px] font-normal text-muted-foreground">{mode.multiplier}</span></span><span className="block truncate text-[11px] text-muted-foreground">{mode.description}</span></span><span className="text-[11px] text-muted-foreground">{percent > 0 ? `${percent}%` : "Start"}</span><ArrowRight className="h-3.5 w-3.5 text-muted-foreground" /></Link>;
}
...
function SingleTrainingAction({ training }: { training: TrainingCardModel }) {
  ...
  return <><div className="mt-4"><div className="mb-1.5 flex items-center justify-between text-[11px] text-muted-foreground"><span>{!training.available ? "In Vorbereitung" : p === 0 ? "Noch nicht gestartet" : `${p} % abgeschlossen`}</span>{training.available && p > 0 ? <span>{p} %</span> : null}</div><div className="h-1.5 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-accent transition-all" style={{ width: `${p}%` }} /></div></div>{training.available && training.scenarioId ? <Link to="/training/$scenarioId" params={{ scenarioId: training.scenarioId }} className="mt-4 inline-flex items-center justify-center gap-1.5 rounded-md bg-accent px-3 py-2 text-sm font-medium text-accent-foreground transition-opacity hover:opacity-90">{p > 0 ? "Fortsetzen" : "Starten"} <ArrowRight className="h-4 w-4" /></Link> : <button disabled title="Dieses Training wird als eigenes Modul ergänzt." className="mt-4 inline-flex items-center justify-center gap-1.5 rounded-md border border-border px-3 py-2 text-sm text-foreground opacity-50"><PlayCircle className="h-4 w-4" /> In Vorbereitung</button>}</>;
}
